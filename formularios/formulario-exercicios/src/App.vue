			...
			<form class="painel">
				...
				<Rotulo nome="Prioridade">
					<select v-model="prioridade">
						<option v-for="p in prioridades" :value="p.codigo"
							:key="p.codigo" :selected="p.codigo === 1">{{p.codigo}} - {{ p.nome }}</option>
					</select>
				</Rotulo>
...
export default {
	...
	computed: {
		...
		nomePrioridade() {
			const pSelect = this.prioridades.filter(p => p.codigo === this.prioridade)[0]
			return pSelect.codigo + ' - ' + pSelect.nome
		}		
	},
	data() {
		return {
